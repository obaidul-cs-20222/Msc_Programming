// CODE FOR COUNTING NUMBER OF LEAF NODES AND NON LEAF NODES
// Approach: for counting the non leaf nodes just recursively call counter that will return the value of
// total number of non leaf nodes in the left and right sub tree
#include <stdio.h>
#include <stdlib.h>
struct node
{
...
int counter(struct node *start)
{
    ...
    {
        return 1 + counter(start->left) + counter(start->right);
    }
}